 ...
 * 获取项目的图片列表（分页）
 */
export async function getImages(projectId, page = 1, pageSize = 20, imageName = '', hasQuestions, hasDatasets) {
  try {
    const whereClause = {
      projectId,
    ...
    // 应用筛选条件
    let filteredData = imagesWithStats;
    if (hasQuestions === 'true') {
      filteredData = filteredData.filter(img => img.questionCount > 0);
    } else if (hasQuestions === 'false') {
      filteredData = filteredData.filter(img => img.questionCount === 0);
    }

    if (hasDatasets === 'true') {
      filteredData = filteredData.filter(img => img.datasetCount > 0);
