...
class BaseClient {
  ...
  async chat(messages, options) {
    ...
    //console.log(']]]]]]LLM base chat]]]]:');
    // const res = await this.client[method](...args);
    //console.log('--- LLM send（JSON）---');
  ...
  async chatStream(messages, options) {
    ...
    //console.log(']]]]]]LLM base chatstream]]]]:');
  ...
  async chatStreamAPI(messages, options) {
    ...
    //console.log(']]]]]]LLM base___1发起流式请求]]]]:');
    // 构建请求数据
    const payload = {
      model: modelName,
      messages: this._convertJson(messages),
    ...
    //console.log(']]]]]]LLM base chatstreamAPI]]]]:');
    // 添加思维链相关参数
    payload.send_reasoning = true;
    payload.reasoning = true;
    //console.log(']]]]]]LLM base__1发起流式请求]]]]:');
    try {
      // 发起流式请求
      ...
      const newStream = new ReadableStream({
        async start(controller) {
          ...
          //console.log(']]]]]]LLM base]]]]:',newStream);
          // 输出文本内容
