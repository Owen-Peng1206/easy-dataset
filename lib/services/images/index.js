...
import { saveQuestions, updateQuestionAnsweredStatus } from '@/lib/db/questions';
...
export async function generateDatasetForImage(projectId, imageId, question, options) {
  try {
    ...
    await updateQuestionAnsweredStatus(projectId, image.id, question, true);

    logger.info(`图片 ${image.imageName} 的问题 "${question}" 已生成数据集`);
